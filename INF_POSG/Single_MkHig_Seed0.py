...
# thetaS = np.array([1.8028, 1.9129, -0.3808 , -0.0252 , 0.0052 ,9.0096,
#                    1.3930,1.4743, -0.3980, -0.0144, 0.0250, 7.1176])
thetaS = np.array([1.8373, 2.0659, -0.5321 , -0.1270 , 0.0436 ,10.7054,
                    1.2737,1.6641, -0.5627, -0.1987, 0.0565, 7.4121])
...
stateID_aw = Ni_aw*4**2+Nj_aw*4**1+disc_inc
stateID_bk = Ni_bk*4**2+Nj_bk*4**1+disc_inc
stateID_hvy = Ni_hvy*4**2+Nj_hvy*4**1+disc_inc
stateID_mcd = Ni_mcd*4**2+Nj_mcd*4**1+disc_inc
stateID_wdy = Ni_wdy*4**2+Nj_wdy*4**1+disc_inc
...
numstates = 5-2 # ni,nj,pop,val,inc
...
states = np.stack([Ni,Nj,dz1],axis=1)
...
F_Ni[0,0] = np.kron(np.array([[1]*4**2+[0]*4**2 + [0] *4**2+[0]*4**2]*4**2 +\
                          [[1]*4**2+[0]*4**2 + [0] *4**2+[0]*4**2]*4**2 + \
                          [[0]*4**2+[1]*4**2 + [0] *4**2+[0]*4**2]*4**2 + \
                          [[0]*4**2+[0]*4**2 + [1] *4**2+[0]*4**2]*4**2 ),np.ones([NX,1,1]))
F_Ni[0,1] =np.kron(np.array([[1]*4**2+[0]*4**2 + [0] *4**2+[0]*4**2]*4**2 +\
                          [[0]*4**2+[1]*4**2 + [0] *4**2+[0]*4**2]*4**2 + \
                          [[0]*4**2+[0]*4**2 + [1] *4**2+[0]*4**2]*4**2 + \
                          [[0]*4**2+[0]*4**2 + [0] *4**2+[1]*4**2]*4**2 ),np.ones([NX,1,1]))
F_Ni[0,2] = np.kron(np.array([[0]*4**2+[1]*4**2 + [0] *4**2+[0]*4**2]*4**2 +\
                          [[0]*4**2+[0]*4**2 + [1] *4**2+[0]*4**2]*4**2 + \
                          [[0]*4**2+[0]*4**2 + [0] *4**2+[1]*4**2]*4**2 + \
                          [[0]*4**2+[0]*4**2 + [0] *4**2+[1]*4**2]*4**2 ),np.ones([NX,1,1]))
F_Ni[1] = F_Ni[0]
...
F_dz3 = np.kron((fz3_4x4[dz1,:])[:,dz1],np.ones([I,J,NX,1,1]))#; % The transition pattern is common across (i,j,mtype)
...
def updateF(P):
  ...
  x =  (4**2)*Ni + (4**1)*nj + dz1 #+ 4*dz2+dz3#;  % index of today's own state
  x0 = (4**2)*0 + (4**1)*Ni + dz1 #+ 4*dz2+dz3#;  % index of today's state for rivals with 0 shops
  ...
  x =  (4**2)*Ni + (4**1)*nj + dz1# +4*dz2+dz3#;  % index of today's own state
  x0 = (4**2)*0 + (4**1)*Ni + dz1 #+4*dz2+dz3#;  % index of today's state for rivals with 0 shops
  x1 =  (4**2)*1 + (4**1)*Ni + dz1 #+4*dz2+dz3#;  % index of today's state for rivals with 1 shops
  ...
  x0 = (4**2)*0 + (4**1)*Ni + dz1 #+4*dz2+dz3#;  % index of today's state for rivals with 0 shops
  x1 = (4**2)*1 + (4**1)*Ni + dz1 #+4*dz2+dz3#;  % index of today's state for rivals with 1 shops
  x2 = (4**2)*2 + (4**1)*Ni + dz1 #+4*dz2+dz3#;  % index of today's state for rivals with 2 shops
  ...
  x1 = (4**2)*1 + (4**1)*Ni + dz1 #+4*dz2+dz3#;  % index of today's state for rivals with 1 shops
  x2 = (4**2)*2 + (4**1)*Ni + dz1 #+4*dz2+dz3#;  % index of today's state for rivals with 2 shops
  x3 = (4**2)*3 + (4**1)*Ni + dz1 #+4*dz2+dz3#;  % index of today's state for rivals with 3 shops
...
def ValueIteration(thetaS,sigmaP,iterpP,iterf,iterc,error = 1e-2,beta = 0.9):
  ...
  thetaS_mcd = np.reshape(np.array([thetaS[2],thetaS[3],thetaS[4],thetaS[5]]),[-1,1])
  thetaS_other = np.reshape(np.array([thetaS[8],thetaS[9],thetaS[10],thetaS[11]]),[-1,1])
  Ni_temp = (Ni).reshape([1,-1])

  r_mcd_a2 = np.stack([Ni *Ni,Ni*Nj, Ni*dz1,-1* np.ones(NS)],axis=1).dot(thetaS_mcd)
  r_mcd_a1 = np.stack([Ni *Ni,Ni*Nj, Ni*dz1, 0* np.ones(NS)],axis=1).dot(thetaS_mcd)
  ...
  r_other_a2 = np.stack([Ni *Ni,Ni*Nj,Ni*dz1,  -1* np.ones(NS)],axis=1).dot(thetaS_other)
  r_other_a1 = np.stack([Ni *Ni,Ni*Nj,Ni*dz1,  0* np.ones(NS)],axis=1).dot(thetaS_other)

  R_new[1,2] =  np.ones([NX,1]).dot(r_other_a2.T)+\
                thetaS[6]*    x_old.dot(Ni_temp)+\
                thetaS[7]*(1-x_old).dot(Ni_temp)
...
def boostrap(seed,
    ...
    if seed==-1:
        samples = np.arange(len(dataX),dtype = int)#
    else:
        np.random.seed(seed)
        samples = np.random.randint(0,len(dataX),len(dataX))#
    print(samples)
...
if seed_ini==0:
    seed_ini=-1
for seed in range(seed_ini,seed_ini +51):
  print(seed)
  ...
  print('{:.4f}&{:.4f}'.format(haha[0],haha[6]))
  print('{:.4f}&{:.4f}'.format(haha[1],haha[7]))
  print('{:.4f}&{:.4f}'.format(haha[2],haha[8]))
  print('{:.4f}&{:.4f}'.format(haha[3],haha[9]))
  print('{:.4f}&{:.4f}'.format(haha[4],haha[10]))
  print('{:.4f}&{:.4f}'.format(haha[5],haha[11]))
  DataRes.append(res22)
  DataRes.append(res22)
  if seed !=-1:
      with open(file_name+'Single_Mk'+str(mk)+'_Seed'+str(seed)+'.txt','wb') as fp:
        pickle.dump(DataRes,fp)
  else:
      #thetaS = res22.x
      with open(file_name+'Single_Mk'+str(mk)+'_Seed.txt','wb') as fp:
        pickle.dump(DataRes,fp)      
