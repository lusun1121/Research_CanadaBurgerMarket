...
% Values for 'counterfactual' index:
%
% 0. Actual initial values of exogenous variables, Z's from stationary
%    distributions.
% 1. Initial income is (1+alpha) times actual final year income.
% 2. Initial minimum wage is (1+alpha) times actual final year minimum
%    wage.
% 3. Initial Z's drawn from stationary distribution of McDonald's for
%    all firms.
% 4. Initial Z's drawn from stationary distributions with rescaled
%    deltas: delta = (1+alpha)*delta.
% 5. Initial Z's for McDonald's drawn from stationary distribution
%    with rescaled AR(1) coefficient (1+alpha)*delta.

function [meanshare] = forwardsimrobust(param, rhoX, sigmaX, data, stdX, specification, numpaths, numperiods, discount, paramS, secondspec, counterfactual, alpha)
...
Nstore = zeros(nummarkets*years,players);
PIstore = zeros(nummarkets*years,players);
PIstorenew = zeros(nummarkets,players);
...
for market = 1:nummarkets
    ...
    % Adjust stationary mean and variance for counterfactual 4 and 5
    deltatmp = delta;
    if counterfactual == 4
        deltatmp = (1+alpha).*deltatmp;
    elseif counterfactual == 5
        deltatmp(4) = (1+alpha)*deltatmp(4);
    end
    stationarymeanAW = (mu(1)+eta(market))/(1-deltatmp(1));
    stationarymeanBK = (mu(2)+eta(market))/(1-deltatmp(2));
    stationarymeanHARV = (mu(3)+eta(market))/(1-deltatmp(3));
    stationarymeanMCD = (mu(4)+eta(market))/(1-deltatmp(4));
    stationarymeanWEND = (mu(5)+eta(market))/(1-deltatmp(5));
    stationaryvarAW = (beta(6)^2)/(1-deltatmp(1)^2);
    stationaryvarBK = (beta(12)^2)/(1-deltatmp(2)^2);
    stationaryvarHARV = (beta(18)^2)/(1-deltatmp(3)^2);
    stationaryvarMCD = (beta(24)^2)/(1-deltatmp(4)^2);
    stationaryvarWEND = (beta(30)^2)/(1-deltatmp(5)^2);
    ...
    % Initialize X.
    if counterfactual == 1
        D = [population(years), (1+alpha)*income(years), value(years), minwage(years)];     
    elseif counterfactual == 2
        D = [population(years), income(years), value(years), (1+alpha)*minwage(years)];
    else
    ...
    for t = 1:numperiods
        ...
        if t == 1
            ...
            if counterfactual == 0
                NinitAW = kron(numAW(1)+alpha,op);
                NinitBK = kron(numBK(1)+alpha,op);
                NinitHARV = kron(numHARV(1)+alpha,op);
                NinitMCD = kron(numMCD(1),op);
                NinitWEND = kron(numWEND(1)+alpha,op);
            else
            ...
            % Initial draws of Z
            if counterfactual == 3
                rpAW = (1+alpha)*(stationarymeanMCD + sqrt(stationaryvarMCD) * re(:,1));
                rpBK = (1+alpha)*(stationarymeanMCD + sqrt(stationaryvarMCD) * re(:,2));
                rpHARV = (1+alpha)*(stationarymeanMCD + sqrt(stationaryvarMCD) * re(:,3));
                rpMCD = stationarymeanMCD + sqrt(stationaryvarMCD) * re(:,4);
                rpWEND = (1+alpha)*(stationarymeanMCD + sqrt(stationaryvarMCD) * re(:,5));
            else
        ...
        PIAWtheta = (PIAW*thetaSr(:,1)).*1.*(PIAW*thetaSr(:,1)>0);
        PIBKtheta = (PIBK*thetaSr(:,2)).*1.*(PIBK*thetaSr(:,2));
        PIHARVtheta = (PIHARV*thetaSr(:,3)).*1.*(PIHARV*thetaSr(:,3));
        PIMCDtheta = (PIMCD*thetaSr(:,4)).*1.*(PIMCD*thetaSr(:,4));
        PIWENDtheta = (PIWEND*thetaSr(:,5)).*1.*(PIWEND*thetaSr(:,5));
        ...
        if market == 1
            Nstore(t,:) = [mean(NAW), mean(NBK), mean(NHARV), mean(NMCD), mean(NWEND)];
            PIstore(t,:) = [mean(PIAWtheta), mean(PIBKtheta), mean(PIHARVtheta), mean(PIMCDtheta), mean(PIWENDtheta)];            
        else
            Nstore((market-1)*years + t,:) = [mean(NAW), mean(NBK), mean(NHARV), mean(NMCD), mean(NWEND)];
            PIstore((market-1)*years + t,:) = [mean(PIAWtheta), mean(PIBKtheta), mean(PIHARVtheta), mean(PIMCDtheta), mean(PIWENDtheta)];
        end    
       
        % Store the last period's discounted profits to avoid double counting.
        if t == numperiods
            PIstorenew(market,:) = [mean(PIAWtheta), mean(PIBKtheta), mean(PIHARVtheta), mean(PIMCDtheta), mean(PIWENDtheta)];  
        end
...
% Each chain's market share across all markets in Canada.
TPI = sum(PIstorenew);
meanshareAW = TPI(1)/sum(TPI);        % Share of A & W's profits nationally.
meanshareBK = TPI(2)/sum(TPI);        % Share of Burger King's profits nationally.
meanshareHARV = TPI(3)/sum(TPI);      % Share of Harvey's profits nationally.
meanshareMCD = TPI(4)/sum(TPI);       % Share of McDonald's profits nationally.
meanshareWEND = TPI(5)/sum(TPI);      % Share of Wendy's profits nationally.

% Store each chain's market share.
meanshare = [meanshareAW, meanshareBK, meanshareHARV, meanshareMCD, meanshareWEND];
