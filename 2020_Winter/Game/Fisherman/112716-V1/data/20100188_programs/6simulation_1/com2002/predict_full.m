...
probit_par=[3.0774	0.111	0.1534	-0.9464	-0.6834	0.0306	-0.0151	0.00223	-0.000048	...
...
%     state_daily(tt-m+2,4)=diesel_price(tt-m+1,2)+sqrt(v_diesel)*randn;       %%%diesel price
    ...
    state_daily(tt,1)= shrimp_price(tt,2);%prize_a
    state_daily(tt,2:3)= daily(tt,5:6);% wspd,wvht
    state_daily(tt,4)=diesel_price(tt,1);%%%diesel price
...
%      +probit_par(15)*state_daily(tt-m+2,3)*indiv(:,4)+ffix1;
     ...
     +probit_par(22)*daily(tt,9)+probit_par(23)*state_daily(tt,2)*indiv(:,4)+probit_par(24)*state_daily(tt,3)*indiv(:,4) ...
     +probit_par(25)*exp(zetan(tt,1));
...
%%WVHT*WVHT	WVHT*WVHT*WVHT	opening1	vess_fixed*WSPD	vess_fixed*WVHT
   ...
   % effort_1=sum(indiv(:,5).*fishing_new_1);%%%new  harvest
    ...
    Total_vessel(tt)=share_new_1;    
    harvest(tt)=sum(har_real_1);
    
     accum1=accum1+harvest(tt);
