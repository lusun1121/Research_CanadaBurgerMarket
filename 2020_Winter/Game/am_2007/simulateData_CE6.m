...
%}
function [choices,iX] = simulateData_CE6(deltaU_type1,deltaU_type2,theta1,capPi,nPeriods,nFirms)

type1 = rand(1,nFirms)<theta1;
%{
...
choices1 = deltaU_type1(iX,1)' > deltaEpsilon;
choices2 = deltaU_type2(iX,1)' > deltaEpsilon;
choices = choices1.*type1 + choices2.*(1-type1);
%{
...
for t = 2:nPeriods
	...
	deltaEpsilon  = random('ev',zeros(1,nFirms),ones(1,nFirms))-random('ev',zeros(1,nFirms),ones(1,nFirms));
	choices = [choices;(deltaU_type1(iX(end,:)+nSuppX*choices(end,:)) > deltaEpsilon).*type1+...
        (deltaU_type2(iX(end,:)+nSuppX*choices(end,:)) > deltaEpsilon).*(1-type1)];
end